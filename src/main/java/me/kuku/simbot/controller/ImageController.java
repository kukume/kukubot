...
import me.kuku.simbot.utils.ImageHandle;
...
public class ImageController {
	...
	@Filter(value = "丢", trim = true, anyAt = true)
	public MessageContent diuAt(GroupMsg groupMsg){
...
//	}
	...
	public MessageContent jiAo(GroupMsg groupMsg){
		Long qq = BotUtils.getAt(groupMsg.getMsgContent());
		String url = acatarUrl(qq);
		return getJiao(url);
	}

	@Filter(value = "mua", trim = true, anyAt = true)
	public MessageContent mua(GroupMsg groupMsg){
		Long qq = BotUtils.getAt(groupMsg.getMsgContent());
		String url = acatarUrl(qq);
		return getMuaGif(url);
	}

	@Filter(value = "摸", trim = true, anyAt = true)
	@Filter(value = "rua", trim = true, anyAt = true)
	public MessageContent mo(GroupMsg groupMsg){
		Long qq = BotUtils.getAt(groupMsg.getMsgContent());
		String url = acatarUrl(qq);
		return getRua(url);
	}
	...
	private MessageContent getDiu(String url){
		...
		try {
			...
			return builder.image(bos.toByteArray()).build();
		} catch (IOException e) {
			return builder.text("图片发送失败：" + e.getMessage()).build();
		}
	}

	private MessageContent getPa(String url){
		MessageContentBuilder builder = messageContentBuilderFactory.getMessageContentBuilder();
		BufferedImage headImage;
		BufferedImage bgImage;
		int hdW = 65;
	...
	private MessageContent getJiao(String url){
		MessageContentBuilder builder = messageContentBuilderFactory.getMessageContentBuilder();
		ByteArrayOutputStream bos = new ByteArrayOutputStream();
		...
		try {
			...
			e.printStackTrace();
			return builder.text("在生成图片时出现了一点点错误").build();
		}
		...
		if (status != GifDecoder.STATUS_OK) {
			return builder.text("在生成图片时出现了一点点错误:" + status).build();
		}
	...
	private MessageContent getRua(String url){
		MessageContentBuilder builder = messageContentBuilderFactory.getMessageContentBuilder();
		int hw = 80;//头像宽度
		//得到头像
	...
	private MessageContent getSuperPa(String url){
		int w = 360;
		int h = 360;
		int delay = 50;
		int hdW = 65;
	...
	private MessageContent getMuaGif(String url){
		int bgWH = 240;//背景宽高
