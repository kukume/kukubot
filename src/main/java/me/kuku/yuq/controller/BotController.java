...
public class BotController {
	...
	public void before(BotActionContext actionContext){
		QqLoginEntity qqLoginEntity = BotUtils.getQQLoginEntity();
	...
	@Action("t未发言")
	@QMsg(at = true)
	public String tNotSpeakByDay(Group group, long qq, QqLoginEntity qqLoginEntity) throws IOException {
		...
		if (qq == Long.parseLong(master)){
			for (Long innerQQ : notSpeakByDay) {
				try {
					...
					qqGroupLogic.deleteGroupMember(qqLoginEntity, qq, group.getId(), true);
				}
			}
			return "踢出成功！！";
