...
public class ToolLogicImpl implements ToolLogic {
	...
	public String ping(String domain) throws IOException {
		...
		if (process != null){
			byte[] bytes = IOUtils.read(process.getInputStream(), true);
			String result;
	...
	public JSONObject luckJson(int index){
		if(luckJson == null){
			...
			try {
				...
				if (is != null) {
					byte[] bytes = IOUtils.read(is, true);
	...
	@Override
	public Result<String> tuLing(String apiKey, String userid, String content) throws IOException {
		JSONObject jsonObject = OkHttpUtils.postJson("http://openapi.tuling123.com/openapi/api/v2",
				OkHttpUtils.addJson("{\"reqType\":0,\"perception\": {\"inputText\": {\"text\": \"" + content
						+ "\"}},\"userInfo\": {\"apiKey\": \"" + apiKey + "\",\"userId\": \"" + userid + "\"}}"));
		Integer code = jsonObject.getJSONObject("intent").getInteger("code");
		String text = jsonObject.getJSONArray("results").getJSONObject(0)
				.getJSONObject("values").getString("text");
		if (code == 10004)
			return Result.success(text);
		else return Result.failure(text);
	}
}
