...
class QqMusicLogic {
    ...
    suspend fun checkQrcode(qqLoginQrcode: QqLoginQrcode): CommonResult<QqMusicEntity> {
        val result = QqQrCodeLoginUtils.checkQrCode(appId, daId, ptAid,
            "https://graph.qq.com/oauth2.0/login_jump", qqLoginQrcode.sig
        )
        if (result.failure()) return CommonResult.failure(code = result.code, message = result.message)
        val urlResult = QqQrCodeLoginUtils.authorize(result.data(), ptAid, "state",
            "https://y.qq.com/portal/wx_redirect.html?login_type=1&surl=https://y.qq.com/"
        )
        if (urlResult.failure()) return CommonResult.failure(urlResult.message)
        ...
        return CommonResult.success(QqMusicEntity().also {
            it.cookie = cookie
        })
    }

    suspend fun loginByPassword(qq: Long, password: String, qqVc: QqPasswordConnectLoginUtils.QqVc? = null): CommonResult<QqMusicEntity> {
        ...
        return if (result.failure()) CommonResult.failure(result.message)
        else {
            val url = result.data()
            val code = MyUtils.regex("(?<=code\\=).*", url)
            val response = OkHttpKtUtils.post(
            ...
            response.close()
            val cookie = OkUtils.cookie(response)
            CommonResult.success(QqMusicEntity().also {
                it.cookie = cookie
            })
        }
    }

    suspend fun sign(qqMusicEntity: QqMusicEntity): CommonResult<Void> {
        ...
        return when (jsonObject["req_0"].getInteger("code")) {
            ...
            else -> CommonResult.failure("qq音乐签到失败，未知错误")
        }
    }

    suspend fun musicianSign(qqMusicEntity: QqMusicEntity): CommonResult<Void> {
    ...
    suspend fun publishNews(qqMusicEntity: QqMusicEntity, content: String): CommonResult<Void> {
        val qqMusicKey = qqMusicEntity.qqMusicKey
    ...
    suspend fun comment(qqMusicEntity: QqMusicEntity, id: Int, content: String): CommonResult<String> {
        val qqMusicKey = qqMusicEntity.qqMusicKey
    ...
    suspend fun replyComment(qqMusicEntity: QqMusicEntity, content: String): CommonResult<Void> {
        ...
        val jsonStr = MyUtils.regex("firstPageData = ", "</sc", str) ?: return CommonResult.failure("获取歌曲失败，cookie已失效！")
    ...
    private suspend fun replyComment(qqMusicEntity: QqMusicEntity, songId: String, cmId: String, content: String): CommonResult<Void>{
    ...
    suspend fun randomReplyComment(qqMusicEntity: QqMusicEntity, content: String): CommonResult<Void> {
    ...
    suspend fun convertGreenDiamond(qqMusicEntity: QqMusicEntity): CommonResult<Void> {
    ...
    suspend fun daySign(qqMusicEntity: QqMusicEntity): CommonResult<Void> {
        ...
        return if (jsonObject.getInteger("code") == 0) {
            ...
            when (innerJsonObject.getInteger("code")){
                ...
                1000 -> CommonResult.failure("qq音乐日签失败，cookie已失效！", null)
                else -> CommonResult.failure("qq音乐日签失败，未知原因！")
            }
        }else CommonResult.failure("sign验证失败！")
    }

    suspend fun shareMusic(qqMusicEntity: QqMusicEntity): CommonResult<Void> {
        val str = OkHttpKtUtils.getStr("https://i.y.qq.com/n2/m/share/profile_v2/index.html",
            OkUtils.headers(qqMusicEntity.cookie, "", UA.MOBILE))
        val jsonStr = MyUtils.regex("firstPageData = ", "</sc", str) ?: return CommonResult.failure("获取歌曲失败，cookie已失效！")
        val jsonObject = JSON.parseObject(jsonStr)
